module Shopr
  class PaymentsController < ApplicationController
    ...
    def create
      ...
      if payment.save
        redirect_to @order, flash: { notice: t('shopr.payments.create_notice') }
      else
    ...
    def destroy
      @payment.destroy
      redirect_to @order, flash: { notice: t('shopr.payments.destroy_notice') }
    end

    def refund
      if request.post?
        @payment.refund!(params[:amount])
        redirect_to @order, flash: { notice: t('shopr.payments.refund_notice') }
      else
        render layout: false
      end
    rescue Shopr::Errors::RefundFailed => e
