module Shopr
  class Order < ApplicationRecord
    ...
    belongs_to :delivery_country, class_name: 'Shopr::Country', foreign_key: 'delivery_country_id'

    # The user who marked the order has shipped
    #
    # @return [Shopr::User]
    belongs_to :shipper, class_name: 'Shopr::User', foreign_key: 'shipped_by'
    ...
    #
    # @return [Array] an array of Shopr::DeliveryService objects
    ...
    #
    # @return [Array] an array of Shopr:DeliveryServicePrice objects
    def delivery_service_prices
      if delivery_required?
        prices = Shopr::DeliveryServicePrice.joins(:delivery_service).where(shopr_delivery_services: { active: true }).order(:price).for_weight(total_weight)
    ...
    def ship!(consignment_number, user = nil)
      run_callbacks :ship do
        ...
        save!
        Shopr::OrderMailer.shipped(self).deliver
      end
    end
  end
end
