module Shopr
  module ApplicationHelper
    ...
    def status_tag(status)
      content_tag :span, t("shopr.orders.statuses.#{status}"), class: "status-tag #{status}"
    end
    ...
    def settings_field(field, options = {})
      default = I18n.t('shopr.settings.defaults')[field.to_sym]
      value = (params[:settings] && params[:settings][field]) || Shopr.settings[field.to_s]
      type = I18n.t('shopr.settings.types')[field.to_sym] || 'string'
      case type
      when 'boolean'
        ''.tap do |s|
          ...
          s << label_tag("settings_#{field}_true", t("shopr.settings.options.#{field}.affirmative", default: 'Yes'))
