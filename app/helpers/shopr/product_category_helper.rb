          ...
          category.children.ordered.each do |child|
            ...
            s << if child == current_category
                   if link_to_current == false
                     ...
                     "#{nested_product_category_spacing_adjusted_for_depth child, relative_depth} &#8627; #{link_to(child.name, [:edit, child]).html_safe} (#{t('shopr.product_category.nesting.current_category')})"
                        end
                 else
                   "#{nested_product_category_spacing_adjusted_for_depth child, relative_depth} &#8627; #{link_to(child.name, [:edit, child]).html_safe}"
                 end
